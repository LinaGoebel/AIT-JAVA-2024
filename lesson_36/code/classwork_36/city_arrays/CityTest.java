...
//    - вставить новый объект в массив, соблюдая порядок сортировки
  ...
  @Test
  void testNative() {   //homework "change the natural sorting to sort by city name (alphabetically)"
    printArray(cities, "Original array: ");
    Arrays.sort(cities);
    printArray(cities, "Native sorting - cities name");
    City city = new City("Berlin", 800_000, null, 0);
    int index = Arrays.binarySearch(cities, city);
    System.out.println("Index = " + index);
  }
  ...
  void testKeepSorted() {
    ...
    City[] citiesCopy = Arrays.copyOf(cities,
        cities.length + 1); // создаем копию массива с длиной на 1 больше
    ...
    System.arraycopy(citiesCopy, index, citiesCopy, index + 1,
        citiesCopy.length - index - 1); // раздвигаем массив
