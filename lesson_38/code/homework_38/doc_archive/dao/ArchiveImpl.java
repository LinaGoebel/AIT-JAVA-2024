...
public class ArchiveImpl implements Archive {
  ...
  public boolean addDocument(Document document) {
    if (document == null || size == documents.length
        || getDocFromFolder(document.getFolderId(), document.getDocumentId()) != null) {
      return false;
    }
    int index = Arrays.binarySearch(documents, 0, size, document, comparator);
    index = index >= 0 ? index
        : -index - 1; // нашли индекс, куда надо вставить элемент массива (документ)
  ...
  public Document[] getAllDocFromFolder(int folderId) {
    return findByPredicate(p -> p.getFolderId()
        == folderId); // передаем в метод findByPredicate сам предикат (условие для поиска)
    // p.getFolderId() == folderId
  }

  private Document[] findByPredicate(
      Predicate<Document> predicate) { // возвращаем массив найденных объектов
  ...
  public Document[] getDocBetweenDate(LocalDate dateFrom, LocalDate dateTo) {
    // O(1)
    Document pattern = new Document(0, Integer.MIN_VALUE, null, null,
        dateFrom.atStartOfDay()); // почему Integer.MIN_VALUE, а не 0
    //ввели объектную переменную, шаблон
    //O(log(n))
    int from = Arrays.binarySearch(documents, 0, size, pattern,
        comparator);//находим индекс начального фото левый край
    //O(1)
    from = from >= 0 ? from : -from - 1;
    // O(1)
    pattern = new Document(0, Integer.MAX_VALUE, null, null,
        LocalDateTime.of(dateTo, LocalTime.MAX));// находим
    // правый край
    // O(log(n))
    ...
    to = to >= 0 ? to : -to - 1;
    // O(n)
    return Arrays.copyOfRange(documents, from,
        to);// диапазон, создаем новый массив с нужными документами
  }
  ...
  public Document[] getDocBetweenDateTwo(LocalDate dateFrom, LocalDate dateTo) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (docDate.isAfter(dateFrom.atStartOfDay()) && docDate.isBefore(
