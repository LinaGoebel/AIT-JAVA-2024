package classwork_46.programmers;
...
public class ProgrammerAppl {
  ...
  private static void printMostPopularTechnologies(List<Programmer> programmers) {
    ...
    Iterable<String> technologies = programmers.stream()
        .map(
            p -> p.getTechnologies()) // преобразование объекта programmer в одно его поле с массивом технологий
  ...
  private static void printMostSkilledProgrammers(List<Programmer> programmers) {
    ...
    for (Programmer programmer : programmers) {
      ...
      skilledProgrammers.putIfAbsent(key,
          new ArrayList<>()); // если такой key встретился 1-й раз, то создаем пустой лист
  ...
  private static List<Programmer> getProgrammers() {
    return List.of(
        ...
        new Programmer("Helen", "Kotlin", "Scala", "Java", "JavaScript", "Pascal"),
